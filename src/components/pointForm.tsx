 ...
 * IndexParametrage
 */
export default class PointForm extends React.Component<Props, State> {
    ...
    for (const element of this.props.options.arrayPoints) {
      setTimeout(() => {
        this.addInput(
          ...
          element.drawGraphicMarker,
          element.shape,
          element.sizeWidth,
          element.sizeHeight,
          element.rotateArrow,
          element.positionShapeX,
          element.positionShapeY,
          element.color,
          //element.mainMetric.refId, element.mainMetric.key, element.mainMetric.keyValue,
          element.textObj,
          element.lowerLimit,
          element.positionParameter,
          element.associateOrientedLinksIn,
          element.associateOrientedLinksOut,
          element.mainMetric
        );
      }, 100);
    }
  ...
  private defineDataCoordinateSpace(): Array<SelectableValue<RegionClass>> {
    const { regionCoordinateSpace } = this.props;
    const dataEspaceCoor: Array<SelectableValue<RegionClass>> = [];
  ...
  private addInput = (
    id?: number,
    label?: string,
    coordinateSpaceAssociate?: SelectableValue<RegionClass>,
    ...
    this.setState((prevState: State) => ({
      arrayPointClass: prevState.arrayPointClass.concat(
        new PointClass(
          num,
          parametrageMetric,
          '',
          ...
          'point' + num.toString(),
          '',
          coordinateSpaceAssociate || {},
          drawGraphicMarker || { label: 'Yes', value: 'true' },
          shape || { label: 'Circle', value: 'circle' },
          sizeWidth || { label: 'Small', value: 'small' },
          sizeHeight || { label: 'Small', value: 'small' },
          rotateArrow || '0',
          positionShapeX || '0',
          positionShapeY || '0',
          color || 'black',
          [],
          [],
          associateOrientedLinkIn || [],
          associateOrientedLinkOut || []
        )
      ),
   ...
   * dsgs
   */
  private addNewFormPoint = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    this.addInput();
  };
   ...
   */
  private deletearrayPointClass(id: number): void {
    const newPointClass: PointClass[] = this.state.arrayPointClass.filter((value: PointClass) => value.id !== id);
    this.setState({
  ...
  private deleteOwnInput = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
   ...
   */
  private generateInputsPoints(): JSX.Element {
    let index = 0;
    ...
    for (const line of arrayInput) {
      ...
      const newInputList: JSX.Element = (
        ...
        >
          <Collapse
            isOpen={this.state.listCollapsePoint[line.id - 1]}
  ...
  defineOptionsSelectPoint(): Array<SelectableValue<string>> {

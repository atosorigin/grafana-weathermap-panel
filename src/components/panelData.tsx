...
class PanelData extends React.Component<IProps> {
	...
	onPanelSelectChanged = (value: SelectableValue<string>) => {
		...
		this.props.onOptionsChange({
			...this.props.options,
			actualPanel: value, panelId: panel.id, panelType: panel.type, panelTargets: newTargets,
		});
	}
		...
		if (props.targets.lenght !== 0) {
			const listTargets = props.targets.map((target: Target, index: number) =>
				<div key={'TargetDataDiv' + index.toString()}>
					...
					<Button id={index.toString()} key={'buttonAdd' + index.toString()}
						onClick={this.loadTarget.bind(this)}>Add Target</Button>
				</div>
			...
			const listTargets = props.targets.map((target: Target, index: number) =>
				<div key={'TargetPromDiv' + index.toString()}>
					...
					<Button variant='danger' id={index.toString()} key={'ButtunDel' + index.toString()}
						onClick={this.deleteTargetProm.bind(this)}>Del Target</Button>
				</div>
			);
			return (
				<div>{listTargets}</div>
			...
			<div>
				...
				<div className='section gf-form-group'>
					<FormField label='target metric' labelWidth={10} key={'TargetPersonalMetric'}
						inputWidth={20} onChange={this.onPersonalTargetChanged.bind(this)}
						type='string' value={options.personalTarget.expr || ''} />

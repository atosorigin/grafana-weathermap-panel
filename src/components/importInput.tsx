...
class ImportInput extends React.Component<IProps, IState> {
	...
	loadMonoLink = (link: any) => {
		const toLoad = new OrientedLinkClass(link.id, link.linkURL, link.meta, link.lowerLimit,
			...
			link.pointIn, link.pointOut, link.regionIn, link.regionOut, link.zIndex, link.pointCPositionX,
			link.pointCPositionY, link.isIncurved);
		console.log(toLoad);
		// Do some test here to see if your already load a coordinatespace with this id
	...
	public onToggleMultiUpload = (isOpen: boolean): void => {
		this.setState({
		...
		return (
			<div>
				<Collapse isOpen={this.state.collapseTotalUrl}
					label='Global Url Import' onToggle={this.onToggleTotalUrl}>
				...
				<Collapse isOpen={this.state.collapseMultiUrl}
					label='Multi Url Import' onToggle={this.onToggleMultiUrl}>
				...
				<Collapse isOpen={this.state.collapseMultiUpload}
					label='Multi Local Import' onToggle={this.onToggleMultiUpload}>
					<div className="Card">
						<DropZone options={this.props.options}

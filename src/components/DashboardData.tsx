...
class DashboardData extends React.Component<IProps> {
	 ...
	 */
	targetsScrapper = () => {
		let scrapper = new FetchButton(this.props);
		let jsonDashboard = this.props.options.Json;
	...
	*/
	loadGlobal = (onClick: { currentTarget: HTMLButtonElement }) => {
		...
		let objectJSON = JSON.parse(this.props.options.contentJson);
		this.targetsScrapper();
		var newList: Array<SelectableValue<string>> = [];
		objectJSON.panels.forEach((panel: any) => {
			newList.push({ value: panel.id.toString(), label: panel.title });
		});
		let newListStep: Array<SelectableValue<string>> = [];

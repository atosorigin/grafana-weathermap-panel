...
export default class DrawArrow extends React.Component<Props, State> {
  constructor(props: Props) {
    super(props);
    this.state = {};
  }

  private defineLimitX(coordinateX: number, coordinateSpace: SelectableValue<RegionClass>) {
  ...
  private defineLimitY(coordinateY: number, coordinateSpace: SelectableValue<RegionClass>) {
  ...
  private defineSizeWidth(size: string): number {
    let sizeInt = 0;
    if (size === 'small') {
      sizeInt = 20;
    } else if (size === 'medium') {
  ...
  private defineSizeHeight(size: string): number {
    let sizeInt = 0;
    if (size === 'small') {
      sizeInt = 5;
    } else if (size === 'medium') {
  ...
  private definePositionArrowX(positionX: number, coordinateSpace: SelectableValue<RegionClass>, defineCenter: number, width: number): number {
    if (coordinateSpace.value === undefined) {
  ...
  private definePositionArrowY(positionY: number, coordinateSpace: SelectableValue<RegionClass>, defineCenter: number, size: number): number {
    if (coordinateSpace.value === undefined) {
  ...
  private defineCenterX(coordinateSpace: SelectableValue<RegionClass>): number {
    let result = 0;
    result = this.props.height / 2;
    return result;
  }

  private defineCenterY(coordinateSpace: SelectableValue<RegionClass>): number {
    let result = 0;
    result = this.props.height / 2;
    return result;
  }

  private defineLabel(color: string, positionArrowX: number, positionArrowY: number, drawGraphicMarker: string, heightArrow: number) {
  ...
  render() {
    ...
    const positionArrowX: number = this.definePositionArrowX(valueInputPositionArrowX, coordinateSpace, defineCenterX, widthArrow);
    const positionArrowY: number = this.definePositionArrowY(valueInputPositionArrowY, coordinateSpace, defineCenterY, sizeArrow);
    ...
    return (
      <div id={this.props.idPoint}>
        {this.defineLabel(color, positionArrowX, positionArrowY, drawGraphicMarker, heightArrow)}
        {this.drawArrow(widthArrow, heightArrow, rotate, positionArrowX, positionArrowY, color, drawGraphicMarker)}
      </div>
    );
  }
}

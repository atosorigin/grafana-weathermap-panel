...
import { SimpleOptions, IImportFile } from '../types';
...
class DropZone extends React.Component<IProps, IState> {
  ...
  onMultiListFileChanged = (event: {currentTarget: HTMLInputElement}) => {
    let newData: IImportFile[] = this.props.options.saveImportFile.slice();
  ...
  multiDeletFile = (onClick: { currentTarget: HTMLButtonElement}) => {
    const isUrl = (url: IImportFile) => url === this.props.options.saveImportFile[parseInt(onClick.currentTarget.id, 10)];
  ...
  multiUploadDisplay = (props: any): JSX.Element => {
		if (props.file.lenght !== 0) {
			const list = props.file.map((file: IImportFile, index: number) =>
				<div key={'MultiFileDiv' + index.toString()}>
            <FormField id={index.toString()} key={'MultiFile' + index.toString()}
						  label={'File'} labelWidth={5}
						inputWidth={20} onChange={this.onMultiListFileChanged.bind(this)} type='string' value={file.name || ''} />
					<Button variant='danger' id={index.toString()} key={'ButtunDel' + index.toString()}
						onClick={this.multiDeletFile.bind(this)}>Del</Button>
				</div>
			);
			return (
				<div>{list}</div>

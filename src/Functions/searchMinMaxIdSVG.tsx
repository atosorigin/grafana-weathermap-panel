...
const initICoord4DInt = (): ICoord4DInt => {
	return { 'xMin': 0, 'xMax': 0, 'yMin': 0, 'yMax': 0 };
};

const searchForD = (str: string) => {
	const coordinate: ICoord4DInt = initICoord4DInt();
...
export const searchMinMaxIdSVG = (idSVG: string): ICoord4DInt | null => {
	const element = document.getElementById(idSVG);
	if (!element) {
		return null;
	}
	const result: ICoord4DInt = initICoord4DInt();
	if (element) {

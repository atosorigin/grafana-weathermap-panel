...
import { ICoord4D } from 'Models/RegionClass';
...
const searchOtherLimitXYAttribute = (
	element: HTMLElement, limit: number[], labelX: string, labelY: string
): boolean => {
	const x: number = parseInt(element.getAttribute(labelX) || '0', 10);
	const y: number = parseInt(element.getAttribute(labelY) || '0', 10);

	if (x !== null && y !== null) {
		if (verifAllLimit(x, limit, true)
			&& verifAllLimit(y, limit, false)) {
			return false;
		}
	}
...
const searchOtherLimitXnYnAttribute = (
	element: HTMLElement,
	limit: number[]
): boolean => {
	...
	if (x1 && x2 && y1 && y2) {
		if (verifAllLimit(x1, limit, true)
			&& verifAllLimit(x2, limit, true)
			&& verifAllLimit(y1, limit, false)
			&& verifAllLimit(y2, limit, false)) {
			return false;
		}
	}
...
export const returnAllId = (coordinate: ICoord4D, baseMap: IBackground): string[] => {
	const arrId: string[] = [];
	const arrIdVerif: string[] = [];
	const listElement: NodeListOf<Element> | undefined = document
		.getElementById(baseMap.idSVG)?.querySelectorAll('*[id]');
